...
const mapOhms = (node, symbols) => {
    ...
    if (node.fn) {
	//console.log('node.fn:',node.fn,'type:',typeof(node.fn),'name:',node.fn.name);
	if (node.fn.name)
...
const streams = {in: [new capture(0,true), new capture(0,true)], out: [new mutval(0), new mutval(0)]}
const controls = {}

export default { postMsg, streams }
