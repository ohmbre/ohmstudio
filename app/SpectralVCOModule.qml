            ...
            Item {
                ...
                Shape {
                    ...
                    ShapePath {
                        ...
                        pathElements: Array.from(Array(ps.length-1).keys()).map(i => Qt.createQmlObject(`import QtQuick 2.13; PathCubic { x: ${ ps[i+1].x }; y: ${ ps[i+1].y }; control1X: ${ ps[i].x }; control2X: ${ ps[i+1].x }; control1Y: ${ graph.toView({y:0}).y }; control2Y: control1Y}`,spath,""))
                    }
                }
                Repeater {
                    id: repeater
