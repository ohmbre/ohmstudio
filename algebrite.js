...
var algebrite = (function(bigInt) {
  ...
  isnumerictensor = function(p) {
    var a, i, n, o, ref;
    if (isnum(p) || p === symbol(SYMBOL_IDENTITY_MATRIX)) {
      return 1;
    }
    if (!istensor(p) && !isnum(p)) {
      return 0;
    }
    n = p.tensor.nelem;
    a = p.tensor.elem;
    ...
    if (operands.length > 1) {
      ...
      for (i = j1 = 0, ref2 = operands.length; 0 <= ref2 ? j1 < ref2 : j1 > ref2; i = 0 <= ref2 ? ++j1 : --j1) {
        if ((i + shift + 1) <= (operands.length - 1)) {
          if (!(isnumerictensor(operands[i + shift]) || isnumerictensor(operands[i + shift + 1]))) {
  ...
  inner = function() {
    ...
    if (istensor(p1) && istensor(p2)) {
      ...
      if (!(isnumerictensor(p1) || isnumerictensor(p2))) {
        push(p1);
        push(p2);
        inv();
        subtract();
  ...
  findDependenciesInScript = function(stringToBeParsed, dontGenerateCode) {
    var allReturnedLatexStrings, allReturnedPlainStrings, bodyForReadableSummaryOfGeneratedCode, cacheKey, cacheMissPenalty, currentStateHash, cyclesDescriptions, deQuotedDep, dependencyInfo, eachDependency, error, frozen, generatedBody, generatedCode, i, indexOfEachReplacement, indexOfPartRemainingToBeParsed, inited, key, l1, len, len1, len2, len3, len4, len5, len6, len7, m1, n, n1, newUserSymbol, o1, origPrintMode, originalUserSymbol, parameters, possiblyCached, q1, r1, readableSummaryOfGeneratedCode, recursedDependencies, ref2, replacementsFrom, replacementsTo, s1, scriptEvaluation, stringToBeRun, t1, testableString, timeStartFromAlgebra, toBeFrozen, toBePrinted, totalTime, u1, userVariablesMentioned, value, variablesWithCycles;
    if (DEBUG) {
  ...
  run = function(stringToBeRun, generateLatex) {
    var allReturnedLatexStrings, allReturnedPlainStrings, cacheKey, collectedLatexResult, collectedPlainResult, currentStateHash, error, errorWhileExecution, frozen, i, indexOfPartRemainingToBeParsed, n, possiblyCached, stringToBeReturned, theErrorMessage, timeStart, timingDebugWrite, toBeFrozen, totalTime;
